...
class jah(FloatLayout):
    ...
    def screen_up(self,time_passed,**kwargs):
    	...
    	if xh>xmax-mh and hd==0 and choixidh<1:
    		...
    		hd=0
    	if xh<0+mh and choixidh<1 and hg==0:
    		xh=0+1.3*mh
    		reb3.play()
    		hg=1
    		vxh*=-1
    		...
    		hg=0
    	if yh>ymax-mh and choixidh<1 and hh==0:
    		yh=ymax-1.3*mh
    		reb3.play()
    		hh=1
    		vyh*=-1
    		...
    		hh=0
    	if yh<0+mh and choixidh<1 and hb==0:
    		reb3.play()
    		yh=0+1.3*mh
    		hb=1
    		vyh*=-1
...
######:
	######:
    	...
    	if collide(pavx,pavy,xh,yh,mdbe/2) and ih<3 and choixidh>0:
    		fpav=0
    		stoph=1
    	if collide(pavx,pavy,xh,yh,mdbe)and stoph==0 and not collide(xh,yh,xb,yb,mdbe) and pbh==0:## test collide:
    		...
    		if fpav<6 and choixidh>0:
    			stoph=1
    		if fpav>19:
    			reb.play()
    		if fh>fpav:
    			fpav=(fh+acch+abs(raddif(radh,radp)))/2
    			#fpav=fh
    	...
    	if collide(pavx,pavy,xb,yb,mdbe/2) and ih<3 and choixidb>0:
    		fpav=0
    		stopb=1
    	if collide(pavx,pavy,xb,yb,mdbe) and stopb==0 and not collide(xh,yh,xb,yb,mdbe) and pbb==0:
    		...
    		if fpav<6 and choixidb>0:
    			stopb=1
    		if fb>fpav:
    			#fpav=fb+accb/10
    			fpav=(fb+accb+abs(raddif(radb,radp)))/2
    			#fpav=fb
    			#vpavx,vpavy=vectoriser(pavx,pavy,xb,yb)
    	if pbb!=0:
    		pbb+=1

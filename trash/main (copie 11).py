...
class jah(FloatLayout):
    ...
    def screen_up(self,time_passed,**kwargs):
    	global test,pavx,xh,pavy,yh,xmax,ymax,xh,yh,xb,yb,h,b,ih,ib,timeappuh,timeappub,timelacheh,timelacheb,choixidh,choixidb,test,pavx,pavy,vpavx,vpavy,fpav,mp,vxh,vxb,vyh,vyb,fh,fb
    	...
    	if collide(pavx,pavy,xh,yh,90):## test collide
    		vpavx,vpavy=vectoriser(pavx,pavy,xh,yh)
    		if fh>fpav:
    			fpav+=fb
    	if collide(pavx,pavy,xb,yb,90):
    		vpavx,vpavy=vectoriser(pavx,pavy,xb,yb)
    		if fb>fpav:
    			fpav+=fb
    	if collide(xh,yh,xb,yb,100):
...
class jahApp(App):
    ...
    def on_start(self):
    	...
    	Clock.schedule_interval(self.root.screen_up, 0.03)   		

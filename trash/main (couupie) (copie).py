...
from kivy.graphics import Color,Rotate, Rectangle, Point, GraphicException
...
class jah(FloatLayout):
    ...
    def __init__(self, **kwargs):
    	...
    	jah.__init__.__init__(self, **kwargs)
    	self.rect_pos_x = 600
    	self.rect_pos_y = 500
    	...
    	with self.canvas:
    		...
    		self.down=Rectangle(source=str(scb)+'.png',pos=(xmax/2+300,ymax/2+100),size=(100,300))
    		with self.canvas:
    			Color(1, 1, 1,groupe='p')
    			#Rotate(origin=self.rotate_origin, angle=90)	
    			self.up=Rectangle(source=str(sch)+'.png',pos=(xmax/2-400,ymax/2-400),size=(100,300))
    			Rotate(origin=self.rotate_origin, angle=00)	
    ...
    def screen_up(self,time_passed,**kwargs):
    	global test,pavx,xh,pavy,yh,xmax,ymax,xh,yh,xb,yb,h,b,ih,ib,timeappuh,timeappub,timelacheh,timelacheb,choixidh,choixidb,test,pavx,pavy,vpavx,vpavy,fpav,mp,vxh,vxb,vyh,vyb,fh,fb,pbt,pbh,pbb,pbd,wh,wb,wg,wd,bh,bb,bg,bd,hh,hb,hg,hd,sch,scb,gagnant,fc,count,rfc,treset,stoph,stopb,start,switchm,engo
    	#fc frame count

...
ib=1
#test passbye
pbt,pbh,pbb,pbd,wh,wb,wg,wd=0,0,0,0,0,0,0,0
fpav=100
vpavy=0.5
vpavx=0.5
mh,mb,mp=76,76,50
...
class jah(FloatLayout):
    ...
    def screen_up(self,time_passed,**kwargs):
    	global test,pavx,xh,pavy,yh,xmax,ymax,xh,yh,xb,yb,h,b,ih,ib,timeappuh,timeappub,timelacheh,timelacheb,choixidh,choixidb,test,pavx,pavy,vpavx,vpavy,fpav,mp,vxh,vxb,vyh,vyb,fh,fb,pbh,pbb,pbd,wh,wb,wg,wd
    	#passby true? trou haut bas deux pbb pbh pbd walls haut b g d 
    	...
    	if xh>xmax-mh:## h et b dans la  limites de la table
    		vxh*=-1
    	if xh<0+mh:
    		vxh*=-1
    	if yh>ymax-mh:
    	...
    	if yh<0+mh:
    		vyh*=-1
    	if xb>xmax-mb:## h et b dans la  limites de la table
    		vxb*=-1
    	if xb<0+mb:
    		vxb*=-1
    	if yb>ymax-mb:
    	...
    	if ib>4:
        	aa,bb,ee=b[ib-2]
        	cc,dd,ff=b[ib]
        	fb=force(aa,bb,ee,cc,dd,ff)
        	vxb,vyb=vectoriser(aa,bb,cc,dd)
		#########
		#7#77
  
    	if collide(pavx,pavy,xh,yh,80+fpav+fh):## test collide
    	...
    	if collide(pavx,pavy,xb,yb,80+fpav+fb):
    		pavx,pavy=pavx+vpavx*fpav*-1,pavy+vpavy*fpav*-1
    	...
    	if collide(xh,yh,xb,yb,80+fb+fh):
...
class jahApp(App):
    ...
    title = 'Test'
    def on_start(self):
    	global test,pavx,xh,pavy,yh,xmax,ymax,xh,yh,xb,yb,h,b,ih,ib,fh,fb,timeappuh,timeappub,timelacheh,timelacheb,choixidh,choixidb,test,pavx,pavy,vpavx,vpavy,fpav
